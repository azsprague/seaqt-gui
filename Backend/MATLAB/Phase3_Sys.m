...
%        Fermi_level = [Fermi_level, Fermi_energies(i) * 1.60218 * 10^(-19)];
    ...
    E_len=length(E_sys)/System_num;       %Getting lengths of individual DOS
    ...
    for i = 1:System_num
        ...
        dN_arr=[]
        for k=1:E_len
            if E_sys((i-1)*E_len+k) < Fermi_energies(i)
                N=N+dNdE_sys_e_single((i-1)*E_len+k);
            end
            E_arr=[E_arr, E_sys((i-1)*E_len+k)];
            dN_arr=[dN_arr, dNdE_sys_e_single((i-1)+E_len+k)];
        end
        f=1.60218e-19;
        boltz=1.380649e-23;
        sum1=0;
        Ef=0;
        m=0;
        while(min(E_arr)+m < max(E_arr))
            ip=min(E_arr)+m;
            m=m+0.0001;
            sum2=sum1;
            sum1=0;
            for k=1:E_len
                sum1=sum1+(dN_arr(k) / (1+exp((E_arr(k)-(ip))*11604.5629/(block_temps(i)))));
            end
            if sum1 > N
                Ef=ip - (E_arr(2)-E_arr(1));
                Ferm_end=-Ef*f;
                break
            end
        end
