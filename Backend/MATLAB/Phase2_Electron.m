...
group_velocities = gui_json_data.electron_group_velocities; 
...
for i = 1:(gui_json_data.number_of_blocks)
    ...
    for j = 1:(length(E_sys_dos))
        ...
        E_sys_e_single(j) = E_sys(j) * 1.60218e-19;       
        % Universal electron DOS in 1/Volume
        dNdE_sys_e_single(j) = E_sys_dos(j) / (1.60218e-19);
...
save tmp/Electron.mat tau_e E_sys_e dNdE_sys_e E_sys_e_single dNdE_sys_e_single
